...
async def search_studies(
    ...
    try:
        # Test Orthanc connection
        test_response = requests.get(
            f"{settings['url']}/system",  # Fixed URL concatenation
            auth=(settings["username"], settings["password"]),
            headers={"Accept": "application/json"},
            timeout=5
        )
        ...
        if query:
            if query.isdigit():
                orthanc_query["Query"]["PatientID"] = f"{query}*"
            else:
                orthanc_query["Query"]["PatientName"] = f"*{query}*"
            orthanc_query["Query"]["StudyDate"] = "19000101-21001231"
        else:
        ...
        find_response = requests.post(
            f"{settings['url']}/tools/find",  # Fixed URL concatenation
            json=orthanc_query,
            auth=(settings["username"], settings["password"]),
            headers={"Accept": "application/json"},
            timeout=10
        ...
        async with db.acquire() as conn:
            for study_id in paginated_study_ids:
                ...
                try:
                    study = study_response.json()
                    # Check if a report exists for this study and current user
                    has_report = await conn.fetchval('''
                        SELECT EXISTS (
                            SELECT 1 FROM reports WHERE study_id = $1 AND created_by = $2
                        )
                    ''', study_id, current_user['username'])
                    studies.append({
                    ...
                    logger.error(f"Invalid JSON response for study {study_id}: {str(e)}")
                    continue
...
async def get_report(report_id: int, current_user: dict = Depends(get_current_user), db: asyncpg.Pool = Depends(get_db)):
    logger.info(f"Fetching report {report_id} by {current_user['username']}")
    await check_permission(current_user['id'], 'view_reports', db)
    async with db.acquire() as conn:
        report = await conn.fetchrow('''
            SELECT r.id, r.study_id, r.patient_name, r.patient_id, r.study_type, r.study_date,
                   ...
                   r.status, r.notes, rs.start_time, rs.end_time
            FROM reports r
            LEFT JOIN report_sessions rs ON r.study_id = rs.study_id AND rs.user_id = (SELECT id FROM users WHERE username = $2)
            WHERE r.id = $1
        ''', report_id, current_user['username'])
        if not report:
            logger.error(f"Report {report_id} not found")
            raise HTTPException(status_code=404, detail="Report not found")
        study_details = await get_study_details(report['study_id'], current_user)
        return {
            'id': report['id'],
            'studyId': report['study_id'],
            'patientName': report['patient_name'],
            ...
            'startTime': report['start_time'].isoformat() if report['start_time'] else None,
            'endTime': report['end_time'].isoformat() if report['end_time'] else None,
            'modality': study_details.get('modalities', 'N/A')
        }
